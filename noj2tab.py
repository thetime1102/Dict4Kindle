...
def main(filename, outfile='tabfile', delimit=None):
    ...
    with codecs.open(filename, 'r', encoding='utf-8') as fh:
        items = re.findall(u'<INDENT=1>[^\*]*ENDL\*', fh.read(), re.MULTILINE)
    ...
    for item in delimited:
        ...
        for l in lines[:-1]:
            if l.startswith(sep_key):
                ...
                pattern = []
                pattern.append(u'<HEAD>(?P<hira>[%s（）]+)</HEAD>' % unicode_char)
                pattern.append(u'(?P<kata>[%s]+)?' % unicode_char)
                pattern.append(u'((?P<accent>\[[0-9]+\]))?')
                pattern.append(u'((?P<accent2>\[[0-9]+\]))?')
                pattern.append(u'((?P<accent3>\[[0-9]+\]))?')
                pattern.append(u'((?P<kanji>【[%s（）]+】))?' % unicode_char)
                pattern.append(u'((?P<kanji2>〖[%s（）]+〗))?' % unicode_char)
                pattern.append(u'((?P<part>（[%s\[\]]+）[%s\[\]]*))?' % (unicode_char, unicode_char))
